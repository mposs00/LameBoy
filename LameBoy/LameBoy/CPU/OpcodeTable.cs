...
/*0x20*/    new Opcode("JR NZ, ${0:X2}",  1, 8,    (ref Registers regs, Memory mem) => { if(regs.Z) return; if(regs.Immediate8 >= 128) regs.PC -= (ushort) (256 - regs.Immediate8); else regs.PC += regs.Immediate8; }),
            ...
            new Opcode("LD (HL+), A",     0, 8,    (ref Registers regs, Memory mem) => { regs.HL = regs.A; regs.HL++; }),
...
/*0xD0*/    new Opcode("RET NC",          0, 20,   (ref Registers regs, Memory mem) => { if(!regs.CA) regs.PC = Pop(ref mem, ref regs); }),
            ...
            new Opcode("RET C",           0, 20,   (ref Registers regs, Memory mem) => { if(regs.CA) regs.PC = Pop(ref mem, ref regs); }),
            new Opcode("RETI",            0, 16,   (ref Registers regs, Memory mem) => { regs.PC = Pop(ref mem, ref regs); regs.Interrupts = true; }),
...
/*0xE0*/    new Opcode("LD $FF00+{0:X2}, A", 1, 12,(ref Registers regs, Memory mem) => { mem.Write8(0xFF00 + regs.Immediate8, regs.A); }),
            ...
            new Opcode("JP (HL)",         0, 4,    (ref Registers regs, Memory mem) => { regs.PC = regs.HL; }),
...
/*0xF0*/    new Opcode("LD A, $FF{0:X2}", 1, 12,   (ref Registers regs, Memory mem) => { regs.A = mem.Read8(0xFF00 + regs.Immediate8); }),
            ...
            new Opcode("OR {0:X2}",       1, 8,    (ref Registers regs, Memory mem) => { regs.A |= regs.Immediate8; SetFlags(ref regs, regs.A == 0, false, false, false); }),
            new Opcode("UNIMP RST 30h",         0, 16,   (ref Registers regs, Memory mem) => { /*TODO*/}),
            new Opcode("UNIMP LD HL, SP+{0:X2}",1, 12,   (ref Registers regs, Memory mem) => { regs.HL = (ushort)(regs.SP + (sbyte)regs.Immediate8); /*TODO SET FLAGS*/ }),
            new Opcode("LD SP, HL",       0, 8,    (ref Registers regs, Memory mem) => { regs.SP = regs.HL; }),
            new Opcode("LD A, ${0:X4}",   2, 16,   (ref Registers regs, Memory mem) => { regs.A = mem.Read8(regs.Immediate16); }),
            new Opcode("EI",              0, 4,    (ref Registers regs, Memory mem) => { regs.Interrupts = true; }),
            new Opcode("UNIMP",           0, 1,    (ref Registers regs, Memory mem) => { /*TODO*/}),
            new Opcode("UNIMP",           0, 1,    (ref Registers regs, Memory mem) => { /*TODO*/}),
